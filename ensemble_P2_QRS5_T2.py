"""
ensemble_P2_QRS5_T2.py
========================
P da Fase2, QRS da Fase5, T da Fase2 — T "pura" (non distillata, riproduce
la convenzione GE T_Onset=QRS_Offset, NESSUNA separazione ST).

Nota: P e T condividono lo stesso modello (Fase2), quindi gli score vengono
calcolati una sola volta e riusati per entrambe le onde — nessun calcolo
duplicato.

USO:
    python ensemble_P2_QRS5_T2.py \
        --model-fase2 /path/best_v3.pt \
        --model-fase5 /path/best_fase5.pt \
        --xml-dir /path/XML_ECGs \
        --output-dir risultati_P2_QRS5_T2

Per restringere al test set ufficiale (evita data leakage):
    python ensemble_P2_QRS5_T2.py \
        --model-fase2 ... --model-fase5 ... --xml-dir ... \
...
# ═════════════════════════════════════════════════════════════════
# ARCHITETTURA EnhancedUNetV3 — verificata strict=True, zero mismatch
# ═════════════════════════════════════════════════════════════════
...
# ═════════════════════════════════════════════════════════════════
# FUSIONE — P da model_p (Fase2), QRS da model_qrs (Fase5), T da model_t (Fase2)
# ═════════════════════════════════════════════════════════════════
...
def predict_ensemble(model_fase2, model_fase5, signals, device, guard_samples=30):
    # Fase2 fornisce sia P che T -> un solo calcolo, riusato per entrambe le onde
    sc_fase2 = compute_scores(model_fase2, signals, device, guard_samples)
    sc_fase5 = compute_scores(model_fase5, signals, device, guard_samples)

    pred_fase2 = sc_fase2.argmax(0).cpu().numpy()
    pred_fase5 = sc_fase5.argmax(0).cpu().numpy()

    pred = np.zeros(MEDIAN_LEN, dtype=np.int64)
    pred[pred_fase2 == 3] = 3   # T da Fase2 (pura, no ST)
    pred[pred_fase2 == 1] = 1   # P da Fase2
    pred[pred_fase5 == 2] = 2   # QRS da Fase5
    return postprocess_mask(pred)


# ═════════════════════════════════════════════════════════════════
# TEST SET UFFICIALE (opzionale)
# ═════════════════════════════════════════════════════════════════
...
# ═════════════════════════════════════════════════════════════════
# VALUTAZIONE CON SALVATAGGIO INCREMENTALE E RIPRESA
# ═════════════════════════════════════════════════════════════════
def eval_all(model_fase2, model_fase5, xml_files, device, guard_samples, output_dir, save_every, resume):
    rows = []
    partial_path = os.path.join(output_dir, "P2_QRS5_T2_xml_partial.csv")
    ...
    for xml_path in xml_files:
        ...
        pred = predict_ensemble(model_fase2, model_fase5, signals, device, guard_samples)
...
def main():
    p = argparse.ArgumentParser(description="Ensemble P=Fase2, QRS=Fase5, T=Fase2 (T pura, no ST) su file XML GE MAC2000")
    p.add_argument("--model-fase2", required=True)
    p.add_argument("--model-fase5", required=True)
    p.add_argument("--xml-dir", required=True)
    p.add_argument("--output-dir", default="risultati_P2_QRS5_T2")
    ...
    for name, path in [("Fase2", args.model_fase2), ("Fase5", args.model_fase5)]:
    ...
    print(f"\n{'-' * 65}\n VALUTAZIONE — {len(xml_files)} file XML — P=Fase2 QRS=Fase5 T=Fase2\n{'-' * 65}")
    df = eval_all(model2, model5, xml_files, device, guard_samples,
                  args.output_dir, args.save_every, resume=not args.no_resume)

    if df is None:
        print(" Nessun file valutato con successo."); return

    csv_path = os.path.join(args.output_dir, "P2_QRS5_T2_xml.csv")
    df.to_csv(csv_path, index=False)

    print(f"\n{'=' * 65}")
    print(f" P=Fase2, QRS=Fase5, T=Fase2 (T pura, no ST) — {len(df)} file")
    print(f"{'=' * 65}")
